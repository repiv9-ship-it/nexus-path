...
const levels: Level[] = [
  ...
  { id: 3, title: 'The Mid-Term Boss', type: 'quiz', xp: 1000, status: 'locked' },
          ...
          return (
            ...
            >
              ...
              <button
                ...
                className={`w-20 h-20 rounded-full z-10 flex items-center justify-center border-b-8 transition-all active:scale-90 relative group
                  ${lvl.status === 'completed' 
                    ? 'gradient-primary border-primary/50 text-primary-foreground' 
                    : lvl.status === 'active' 
                      ? 'bg-card border-muted text-primary shadow-[0_10px_30px_rgba(99,102,241,0.2)]' 
                      : 'bg-muted border-muted/50 text-muted-foreground'
                  }`}
              >
                {lvl.status === 'active' && (
                  <div className="absolute -top-12 gradient-primary text-primary-foreground px-3 py-1 rounded-lg font-black text-[10px] uppercase animate-bounce whitespace-nowrap">
                    Start!
                  </div>
                )}
              ...
              <p className={`mt-3 font-black text-xs uppercase tracking-tighter text-center max-w-[100px] ${
