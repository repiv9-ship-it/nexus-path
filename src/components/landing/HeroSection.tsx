...
export function HeroSection({ onGetStarted, onExploreCourses }: HeroSectionProps) {
  return (
    <section className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 py-24 pt-32 overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 bg-sidebar" />
      <div className="absolute top-[-20%] left-[-10%] w-[60%] sm:w-[50%] h-[50%] bg-primary/30 blur-[100px] sm:blur-[150px] rounded-full" />
      <div className="absolute bottom-[-20%] right-[-10%] w-[60%] sm:w-[50%] h-[50%] bg-secondary/30 blur-[100px] sm:blur-[150px] rounded-full" />
      
      {/* Grid Pattern */}
      <div className="absolute inset-0 bg-[linear-gradient(rgba(99,102,241,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(99,102,241,0.03)_1px,transparent_1px)] bg-[size:40px_40px] sm:bg-[size:60px_60px]" />
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center space-y-6 sm:space-y-8">
        ...
        <div className="space-y-4 sm:space-y-6">
          <h1 className="text-4xl sm:text-6xl md:text-8xl font-black italic tracking-tighter text-primary-foreground leading-none">
            UNILINGO
          </h1>
          <p className="text-base sm:text-xl md:text-2xl font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] text-primary/80">
            The Academic Nexus
          </p>
          <p className="text-base sm:text-lg md:text-xl text-sidebar-foreground/60 max-w-2xl mx-auto leading-relaxed px-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center pt-4 sm:pt-6 px-4">
          ...
          <Button 
            ...
            variant="outline"
            size="lg"
            className="bg-sidebar-foreground/5 border-sidebar-foreground/20 text-sidebar-foreground font-black text-base sm:text-lg px-6 sm:px-8 py-5 sm:py-6 rounded-xl sm:rounded-2xl hover:bg-sidebar-foreground/10 transition-all w-full sm:w-auto"
          >
        ...
        <div className="flex flex-wrap justify-center gap-6 sm:gap-8 pt-8 sm:pt-12">
          {[
            ...
            <div key={i} className="text-center">
              ...
              <p className="text-xs sm:text-sm font-bold text-sidebar-foreground/50 uppercase tracking-wider">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
